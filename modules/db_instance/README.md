...
| <a name="input_replica_mode"></a> [replica\_mode](#input\_replica\_mode) | Oracle instances specifies replica is mounted or open-read-only. Mounted does not accept user connections and is mainly used for cross-region DR. Read-only incorporates the ACtive Data Guard and applies changes from the source instance to all read replicas. | `string` | `null` | no |
